...
function parseList(raw: unknown): TrackListSession {
	...
	if (isSortDirection(record.sortDirection))
		next.sortDirection = record.sortDirection;
...
function parseSession(raw: unknown): AppSession {
	...
	if (typeof record.consoleOpen === "boolean")
		next.consoleOpen = record.consoleOpen;
	if (record.library && typeof record.library === "object") {
		const library = record.library as Record<string, unknown>;
		if ("list" in library) {
			if (isBrowseMode(library.browseMode))
				next.library.browseMode = library.browseMode;
			next.library.selectedPlaylistId = asNullableId(
	...
	if (record.rekordbox && typeof record.rekordbox === "object") {
		...
		if (isBrowseMode(rekordbox.browseMode))
			next.rekordbox.browseMode = rekordbox.browseMode;
