...
function isTrackPage() {
	...
	return !segments.some((segment) =>
		NON_TRACK_SEGMENTS.has(segment.toLowerCase()),
	);
}
...
function getCurrentPageUrl() {
	const href = isPlaylistPage()
		? getPageHrefForDetection()
...
function getPlaylistActionBarControls(container) {
	const directControls = [...container.children].filter(
		isPlaylistActionBarControl,
	);
...
function findPrimaryMuiActionContainer() {
	...
	for (const container of collectElementsDeep(
		...
		const visibleButtons = [
			...container.querySelectorAll("button.MuiIconButton-root"),
...
function extractTrackUrlFromItem(item) {
	const permalinkLink = item.querySelector(
		'a[href*="/"][href]:not([href^="http"])',
	);
	if (!permalinkLink) {
		return null;
	}
	...
	if (
		segments.some((segment) => NON_TRACK_SEGMENTS.has(segment.toLowerCase()))
	) {
		return null;
	}
...
function extractTrackUrlFromPlayableTile(wrapper) {
	const tile =
		wrapper.closest(
			".playableTile, .soundBadge, .soundList__item, li, article",
